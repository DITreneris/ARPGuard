...
class IndexGenerator:
    ...
    def _process_document(self, file_path: Path) -> None:
        """Process a document and extract index information."""
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                self.index['authors'][author].append(doc_id)
                
                self.index['metadata']['total_documents'] += 1
                self.index['metadata']['total_categories'] = len(self.index['categories'])
                self.index['metadata']['total_tags'] = len(self.index['tags'])
                self.index['metadata']['total_authors'] = len(self.index['authors'])
    ...
    def generate(self) -> None:
        ...
        print(f"Total documents: {self.index['metadata']['total_documents']}")
        print(f"Total categories: {self.index['metadata']['total_categories']}")
        print(f"Total tags: {self.index['metadata']['total_tags']}")
        print(f"Total authors: {self.index['metadata']['total_authors']}")

def main():
    if len(sys.argv) != 3:
        print("Usage: python generate_index.py <docs_directory> <base_url>")
        sys.exit(1)
    
    docs_dir = sys.argv[1]
    base_url = sys.argv[2]
    
    generator = IndexGenerator(docs_dir, base_url)
    generator.generate()
