...
class SearchGenerator:
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for term in terms:
                    if term not in self.search['terms']:
                        self.search['terms'][term] = []
                    self.search['terms'][term].append(doc_id)
                
                self.search['metadata']['total_documents'] += 1
                self.search['metadata']['total_terms'] += len(terms)
    ...
    def generate(self) -> None:
        ...
        # Save search index
        search_file = self.docs_dir / 'search' / 'index.json'
        search_file.parent.mkdir(exist_ok=True)
        
        with open(search_file, 'w', encoding='utf-8') as f:
            json.dump(self.search, f, indent=2)
        
        print(f"Search index generated: {search_file}")
        print(f"Total documents indexed: {self.search['metadata']['total_documents']}")
        print(f"Total terms indexed: {self.search['metadata']['total_terms']}")

def main():
    if len(sys.argv) != 3:
        print("Usage: python generate_search.py <docs_directory> <base_url>")
        sys.exit(1)
    
    docs_dir = sys.argv[1]
    base_url = sys.argv[2]
    
    generator = SearchGenerator(docs_dir, base_url)
    generator.generate()
